...
OUTDIR = Path(__file__).resolve().parent / "arena_results"
...
DISCOVERY_DIRS = [
    ROOT / "artifacts" / "bundles" / "current" / "public_agents" / "elite",
    ROOT / "candidates",
    ROOT / "public_agents",
]
...
def discover_paths(explicit: List[str], include_root_agents: bool) -> List[Path]:
    ...
    e11 = ROOT / "artifacts" / "bundles" / "current" / "public_agents" / "elite" / "prvsiyan_frontier" / "raw" / "main.py"
    if e11.is_file():
        paths.append(e11)
    ...
    if include_root_agents:
        ...
        paths.extend(sorted(experiments.glob("e*/agent.py")))
        paths.extend(sorted((experiments / "legacy").rglob("agent_*.py")))
        paths.extend(sorted(experiments.rglob("raw_e*.py")))
