...
def find_base() -> Path:
    candidates = [
        REPO_ROOT / "artifacts" / "bundles" / "current",
        REPO_ROOT / "archive" / "bundles" / "kaggriculture_elite_bundle_FINAL",
    ]

    try:
        candidates += [
            p for p in (REPO_ROOT / "artifacts" / "bundles").iterdir()
    ...
    raise SystemExit(
        "Could not find the current elite bundle under artifacts/bundles/current."
    )
