"""
E28 — scale-routed state-based M-family agent.
...
# M-family medians from 84 current leaderboard runs.
# The family is best modelled as:
#   fixed-ish total scale trajectory + shop-conditioned composition.
CROP_TOTAL_BY_DAY = [
    16,19,20,20,20,20,33,38,38,53,
    56,57,57,57,57,57,57,57,57,57,
    56,56,56,56,55,55,55,55,36,7,
]
...
def _animal_targets(obs):
    ...
    # Replay-derived response preferences. These are composition signals only;
    # the total herd size is controlled by HERD_TOTAL_BY_DAY.
    raw = {
    ...
    # Keep the opening minima, then allocate remaining slots greedily toward
    # the largest replay-derived target deficits.
    target = {"COW":2, "SHEEP":3, "GOOSE":0}
    slots = max(0, total - sum(target.values()))
    ...
    while slots > 0:
        ...
        for a in ("COW","SHEEP","GOOSE"):
            ...
            if deficit > 0:
                # Prefer cows/sheep over geese at equal deficit because the
                # current M population's strongest routing signal is MILK/WOOL.
                bias = 0.20 if a == "COW" else 0.15 if a == "SHEEP" else 0.0
                candidates.append((deficit + bias, a))

        if not candidates:
            # If raw targets are already reached, use residual slots on the
            # stronger of MILK/WOOL rather than inflating geese.
...
def _desired_crop_map(obs, pasture_positions, coop_positions):
    ...
    # Preserve live ongoing crops because replacing them before exhaustion throws
    # away future yields.
    ...
    # Replay-derived early footprint.
    if day <= 2:
        for p in MELON_OPENING:
            if p not in reserved and _unlocked(obs, p):
                desired[p] = "MELON"
        for p in EARLY_STRAWBERRY_2:
            if p not in reserved and _unlocked(obs, p):
                desired[p] = "STRAWBERRY"
        for p in EARLY_WHEAT_6:
            if (
                len(desired) < total_target
    ...
    # Shop-conditioned phase composition.
    phase_crop = None
    phase_target = 0
    ...
    # Keep remaining early melons until harvested; do not replant after day8.
    if day <= 8:
        for p in MELON_OPENING:
            tile = _tile(obs, p)
            if (
    ...
    if phase_crop:
        existing = sum(1 for c in desired.values() if c == phase_crop)
        for p in _crop_order(obs, phase_crop, reserved):
            if len(desired) >= total_target or existing >= phase_target:
                break
            if p in desired:
                continue
            desired[p] = phase_crop
            existing += 1

    # Wheat is the residual M-family backbone. Crucially, stop at the empirical
    # total footprint rather than filling every unlocked square.
    if day <= 28:
        for p in _crop_order(obs, "WHEAT", reserved):
            if len(desired) >= total_target:
                break
...
def _build_jobs(obs):
    ...
    live_crop_total = sum(_count_crops(obs).values())
    crop_scale_target = CROP_TOTAL_BY_DAY[min(day, len(CROP_TOTAL_BY_DAY)-1)]
    expansion_mode = live_crop_total + 5 < crop_scale_target
    ...
    for y, row in enumerate(farm.get("tiles") or []):
        for x, tile in enumerate(row):
            ...
            if tile.get("animal") in ANIMALS:
                ...
                if not expansion_mode and not tile.get("cared_today", False):
                    jobs.append(_job("CARE", p, ["CARE"], 4.5))

                if not expansion_mode and tile.get("fertilizer_available"):
                    jobs.append(_job("COLLECT", p, ["COLLECT_FERTILIZER"], 6.0))
    ...
    for p in pasture_pos:
        ...
        if tile is None and build_slots > 0:
            jobs.append(_job("BUILD_PASTURE", p, ["BUILD_PASTURE"], -8.0 if expansion_mode else -5.0))
            build_slots -= 1
    ...
    for p in coop_pos:
        tile = _tile(obs, p)
        if tile is None and build_slots > 0:
            jobs.append(_job("BUILD_COOP", p, ["BUILD_COOP"], -7.0 if expansion_mode else -4.0))
            build_slots -= 1
        elif isinstance(tile, dict) and tile.get("kind") == "WEED":
...
def _market_orders(obs):
    """Live market scheduler with explicit capital/order-slot priorities.

    Current M-family replays show that early-day market slots are not fungible:
    hands must be rehired every day, land must be secured after shop reveals,
    and production investment follows only after enough labour/capital exists.

    Order priority:
      1. one/two live SELL orders to create cash;
      2. HIRE toward today's hand target, leaving room for other work;
      3. BUY_LAND at the family expansion window;
      4. wheat feed reserve;
      5. one animal toward the routed herd composition;
      6. seeds toward the fixed total crop footprint.
    """
    ...
    # Exact day-0 family opening.
    if step == 0:
        return (
            [["HIRE"] for _ in range(4)]
            + [["BUY_PRODUCT","WHEAT",3]]
            + [["BUY_ANIMAL","COW",2]]
            + [["BUY_ANIMAL","SHEEP",3]]
            + [["BUY_SEED","MELON",6]]
    ...
    # ---------- live sells ----------
    herd_now = sum(animals[a] for a in ANIMALS)
    wheat_reserve = 0 if day >= 28 else max(2, herd_now)
    fert_reserve = 0 if day >= 28 else (4 if day >= 10 else 0)
    ...
    # Put at least one sale first whenever possible so later atomic HIRE/LAND
    # orders can use the proceeds from earlier order indices.
    orders = []
    for _, order in sell_candidates[:2]:
        orders.append(order)
        if len(orders) >= 2:
            break
    ...
    expected_cash = money
    for o in orders:
        expected_cash += int(o[2]) * float(prices.get(o[1],0) or 0)

    # ---------- labour ----------
    target_hands = HANDS_BY_DAY[min(day, len(HANDS_BY_DAY)-1)]
    current_hires = int(farm.get("hires_today",0) or 0)
    missing_hires = max(0, target_hands - current_hires)

    # Re-attempt throughout the day, not only hours 0-2. Keep two order slots
    # for land/production so high-hand days do not starve the rest of the market.
    max_hires_this_turn = max(0, min(8, 10 - len(orders) - 2))
    hire_n = min(missing_hires, max_hires_this_turn)

    # HIRE costs depend only on hires already made today.
    while hire_n > 0:
        cost = _hire_cost(current_hires + hire_n) - _hire_cost(current_hires)
        if cost <= expected_cash:
            break
        hire_n -= 1

    if hire_n > 0:
        orders.extend([["HIRE"] for _ in range(hire_n)])
        expected_cash -= _hire_cost(current_hires + hire_n) - _hire_cost(current_hires)

    # If today's workforce is still below target, capital-intensive expansion is
    # deferred until a later turn. This avoids the E27 low-hand feedback loop.
    workforce_ready = current_hires + hire_n >= target_hands

    # ---------- land ----------
    unlocked = list(farm.get("unlocked_quadrants") or [])
    land_due = None
    if len(unlocked) == 1 and step >= 150:
        land_due = LAND_PRICES[0]
    elif len(unlocked) == 2 and step >= 220:
        land_due = LAND_PRICES[1]

    if (
        workforce_ready
        and land_due is not None
        and len(orders) < 10
        and expected_cash >= land_due
    ):
        orders.append(["BUY_LAND"])
        expected_cash -= land_due
        # This order changes the board next turn; don't spend the rest of the
        # reserve on animals if cash is already tight.

    # ---------- feed ----------
    wheat_have = int(shed.get("WHEAT",0) or 0) + int(carried.get("WHEAT",0) or 0)
    wheat_need = max(0, herd_now - wheat_have)

    if workforce_ready and wheat_need > 0 and len(orders) < 10 and day < 29:
        buy_price = float(prices.get("WHEAT",25) or 25) + 3
        n = min(wheat_need, int(expected_cash // max(1,buy_price)))
        if n > 0:
            orders.append(["BUY_PRODUCT","WHEAT",n])
            expected_cash -= n * buy_price

    # ---------- composition-aware animal growth ----------
    # Only buy when workforce is ready and no imminent land purchase is still
    # unfunded. One animal per turn is enough to track the observed trajectory.
    land_still_unfunded = land_due is not None and len(farm.get("unlocked_quadrants") or []) < 3 and expected_cash < land_due

    if workforce_ready and not land_still_unfunded and len(orders) < 10 and day <= 17:
        deficits = []
        for a in ("COW","SHEEP","GOOSE"):
            deficit = max(0, animal_targets[a] - animals[a])
            if deficit > 0:
                deficits.append((deficit, a))
        ...
        for _, a in deficits:
            room = (
                pasture_capacity - pasture_animals
                if a in ("COW","SHEEP")
                else coop_capacity - animals["GOOSE"]
            )
            if room <= 0 or expected_cash < ANIMAL_COST[a]:
                continue
            orders.append(["BUY_ANIMAL",a,1])
            expected_cash -= ANIMAL_COST[a]
            break

    # ---------- seed deficits from current state ----------
    if workforce_ready and len(orders) < 10 and day <= 28:
        pasture_pos, coop_pos = _desired_structure_positions(obs, animal_targets)
        desired = _desired_crop_map(obs, pasture_pos, coop_pos)
        desired_counts = Counter(desired.values())

        # Buy toward the *total footprint* rather than keeping an arbitrary tiny
        # seed buffer. Batch sizes reduce pressure on the 10-order market cap.
        crop_priority = (
            ("MELON","STRAWBERRY","WHEAT") if day <= 8
            else ("STRAWBERRY","WHEAT","TOMATO") if day <= 13
            else ("TOMATO","WHEAT","STRAWBERRY") if day <= 20
            else ("CARROT","WHEAT","TOMATO")
        )

        for crop in crop_priority:
            if crop not in SEED_COST or len(orders) >= 10:
                continue
            ...
            batch_cap = 6 if crop == "WHEAT" else 3 if crop in ("CARROT","MELON") else 2
            n = min(deficit, batch_cap, int(expected_cash // SEED_COST[crop]))
            if n <= 0:
                continue

            orders.append(["BUY_SEED",crop,n])
            expected_cash -= n * SEED_COST[crop]

    TELEMETRY["market_orders"] += len(orders)
    return orders[:10]
