...
E23/E24/E25 remain historical experiments.
"""
...
# Six-melon opening tiles, deliberately separate from the five core pastures.
MELON_OPENING = [
    (3,0),(4,0),(2,1),(3,1),(4,1),(1,2),
]
...
def _animal_targets(obs):
    ...
    wool = min(3, d.get("WOOL", 0))
    egg  = min(3, d.get("EGG", 0))

    cow_full = (5, 8, 10, 13, 14)[milk]
    sheep_full = (3, 9, 15, 18)[wool]
    goose_full = (2, 4, 6, 7)[egg]

    # Opening is fixed 2C3S. The shop signal starts changing capital allocation
    ...
    # Yarn-heavy M routes spend capital/space on sheep rather than geese.
    if wool >= 2:
        goose = min(goose, 3)
...
def _desired_structure_positions(obs, targets):
    ...
    pasture_need = targets["COW"] + targets["SHEEP"]
    coop_need = targets["GOOSE"]
    ...
    # Keep coops off positions already reserved as pasture.
    pset = set(pastures)
    coops = [
...
def _desired_crop_map(obs, pasture_positions, coop_positions):
    ...
    # Preserve live ongoing strawberries/tomatoes as occupied production assets.
    ...
    # Every remaining workable tile is the wheat backbone until the last days.
    if day <= 26:
...
def _build_jobs(obs):
    ...
    # 2. Build the structures demanded by the current shop-conditioned herd target.
...
def _market_orders(obs):
    ...
    for animal in ("COW","SHEEP","GOOSE"):
        ...
        if animal in ("COW","SHEEP"):
            room = max(0, pasture_capacity + 2 - pasture_animals)
        else:
            room = max(0, coop_capacity + 1 - animals["GOOSE"])

        n = min(want, room, int(cash // ANIMAL_COST[animal]))
        if n > 0:
            buys.append(["BUY_ANIMAL",animal,n])
            cash -= n*ANIMAL_COST[animal]
            animals[animal] += n
            if animal in ("COW","SHEEP"):
                pasture_animals += n

    # Seed purchases are based on current crop deficit, not on a recorded turn.
    phase = None
    if day <= 2:
        phase = "MELON"
    elif day <= 13:
    ...
    if phase:
        target = crop_targets.get(phase,0)
        deficit = max(0, target - crops.get(phase,0) - seeds.get(phase,0))
        n = min(deficit, 6, int(cash // SEED_COST[phase]))
        if n > 0:
            buys.append(["BUY_SEED",phase,n])
            cash -= n*SEED_COST[phase]

    # Wheat is the residual/default crop.
    if day <= 26 and seeds.get("WHEAT",0) < 5:
        n = min(5-seeds.get("WHEAT",0), int(cash // SEED_COST["WHEAT"]))
        if n > 0:
