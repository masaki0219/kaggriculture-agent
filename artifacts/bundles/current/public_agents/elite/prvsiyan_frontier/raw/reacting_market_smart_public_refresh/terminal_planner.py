    ...
    try:
        ...
        for sweep in range(passes):
            ...
            if not improved or simulations >= max_simulations:
                break
        # Use any remaining fixed simulation budget for one targeted second look at
        # actor 10. This preserves the normal one-pass search and only re-evaluates
        # the worker that can become newly useful after later actors have moved.
        if passes == 1 and n > 10 and simulations < max_simulations:
            actor = 10
            winner = None
            for _, offset, route, bundle_count in _proposals(best, actor, prices, proposals_per_actor):
                if simulations >= max_simulations:
                ...
                score = _value(evaluated, prices)
                if score > best_value and dominates(evaluated, baseline):
                    required = {(tuple(e["xy"]), e["op"], e["actor"]): e["acquired"]
            ...
            if winner:
                current, offset, bundle_count = winner
                best = simulate(obs, config, current, detailed=True)
                changes.append({"pass": 1, "actor": actor, "from_step": START + offset,
                                "resource_bundles": bundle_count, "estimated_stock_value": best_value})
