"""
E27 — trajectory-corrected state-based M-family agent.
...
def _animal_targets(obs):
    ...
    # Empirical max-herd medians from the 84-run M corpus.
    cow_full = (5, 7, 10, 13, 14)[milk]
    sheep_full = (3, 10, 15)[wool]

    # Goose response is substantially weaker/noisier than cows/sheep.
    # Keep it conservative; do not let EGG shops dominate the farm.
    goose_full = (1, 2, 3, 5, 5)[egg]

    # The opening is fixed. Specialisation starts only after expansion.
    if day < 6:
        cow, sheep, goose = 2, 3, 0
    elif day < 9:
        cow = min(cow_full, 7)
        sheep = min(sheep_full, 7)
        goose = min(goose_full, 2)
    elif day < 12:
        cow = min(cow_full, 10)
        sheep = min(sheep_full, 10)
        goose = min(goose_full, 3)
    else:
        cow, sheep, goose = cow_full, sheep_full, goose_full

    # Actual M runs: median max total herd ~17, 90th percentile ~20.
    # Cap the planner at 20 rather than summing independent response maxima.
    pasture_cap = 18
    if cow + sheep > pasture_cap:
        if milk > wool:
            sheep = max(3, pasture_cap - cow)
        elif wool > milk:
            cow = max(5, pasture_cap - sheep)
        else:
            # Keep the larger demand-driven specialization but fit the cap.
            while cow + sheep > pasture_cap:
                if cow - 5 >= sheep - 3 and cow > 5:
                    cow -= 1
                elif sheep > 3:
                    sheep -= 1
                else:
    ...
    goose = min(goose, max(0, 20 - cow - sheep))
...
def _desired_crop_map(obs, pasture_positions, coop_positions):
    ...
    # Preserve existing ongoing crops as productive assets.
    ...
    # Days 0-2: replay-derived NW opening = 12 melon / 6 wheat / 2 strawberry.
    if day <= 2:
        ...
        for p in EARLY_WHEAT_6:
            if p not in reserved and _unlocked(obs, p) and p not in desired:
                desired[p] = "WHEAT"
        return desired

    # Days 3-5: before land 2 the median footprint is ~12 melon + 8 strawberry.
    if day <= 5:
        for p in MELON_OPENING:
            if p not in reserved and _unlocked(obs, p):
                desired[p] = "MELON"
        for p in EARLY_STRAWBERRY_8:
            if p not in reserved and _unlocked(obs, p):
                desired[p] = "STRAWBERRY"
        # Fill any accidental gap with wheat.
        for p in _crop_order(obs, "WHEAT", reserved):
            if p not in desired:
                desired[p] = "WHEAT"
        return desired

    # Days 6-8: land 2 opens; retain the 12-melon block and expand strawberry
    # according to the first two/three shops. Wheat fills residual acreage.
    if day <= 8:
        for p in MELON_OPENING:
            if p not in reserved and _unlocked(obs, p):
                desired[p] = "MELON"

        target = targets["STRAWBERRY"]
        existing = sum(1 for c in desired.values() if c == "STRAWBERRY")
        for p in _crop_order(obs, "STRAWBERRY", reserved):
            if existing >= target:
                break
            if p in desired:
                continue
    ...
    # Day 9+: melons are no longer replanted. Strawberry dominates the early
    # midgame, then tomato, then carrot, with wheat always the residual crop.
    phase_crop = None
    phase_target = 0

    if day <= 13:
        phase_crop = "STRAWBERRY"
        phase_target = targets["STRAWBERRY"]
    elif day <= 20:
        phase_crop = "TOMATO"
        phase_target = targets["TOMATO"]
    elif day <= 27:
        phase_crop = "CARROT"
        phase_target = targets["CARROT"]

    if phase_crop:
        existing = sum(1 for c in desired.values() if c == phase_crop)
        for p in _crop_order(obs, phase_crop, reserved):
            if existing >= phase_target:
                break
            if p in desired:
                continue
...
def _market_orders(obs):
    ...
    crop_targets = _crop_targets(obs)
    ...
    # Budget with expected sale proceeds.
    cash = money + sum(0.8 * o[1][2] * float(prices.get(o[1][1],0) or 0) for o in sell_candidates)

    buys = []

    # Day-0 family opening exactly at the policy level.
    if step == 0:
        buys = (
    ...
    # Hires are daily workers; spread large targets across first three hours.
    ...
    # Expansion is tied to revealed shop information.
    ...
    # Keep enough wheat for today's herd. Buy only the shortfall.
    ...
    # Buy toward shop-conditioned herd targets, but only when structure capacity
    # is nearly available. This makes the policy state-based rather than timed.
    pasture_capacity = structures["PASTURE"]
    coop_capacity = structures["COOP"]
    pasture_animals = animals["COW"] + animals["SHEEP"]

    # M-core replays usually add animals incrementally rather than buying the
    # whole target gap in one turn. Buy at most one animal per turn.
    deficits = []
    ...
    # Seed purchases follow the replay trajectory and are deliberately paced.
    # M6 is only the day-0 tranche; the family grows to ~12 live melons.
    if day <= 5:
        deficit = max(0, 12 - crops.get("MELON",0) - seeds.get("MELON",0))
        n = min(deficit, 2, int(cash // SEED_COST["MELON"]))
        if n > 0:
            buys.append(["BUY_SEED","MELON",n])
            cash -= n*SEED_COST["MELON"]

    # Two strawberries exist even before the first shop; after that expansion
    # is gradual (typically one seed purchase at a time in the source runs).
    if day <= 2:
        straw_target = 2
    elif day <= 5:
        straw_target = 8
    elif day <= 13:
        straw_target = crop_targets["STRAWBERRY"]
    else:
        straw_target = 0

    if straw_target > 0:
        deficit = max(0, straw_target - crops.get("STRAWBERRY",0) - seeds.get("STRAWBERRY",0))
        n = min(deficit, 1, int(cash // SEED_COST["STRAWBERRY"]))
        if n > 0:
            buys.append(["BUY_SEED","STRAWBERRY",n])
            cash -= n*SEED_COST["STRAWBERRY"]

    if 14 <= day <= 20:
        deficit = max(0, crop_targets["TOMATO"] - crops.get("TOMATO",0) - seeds.get("TOMATO",0))
        n = min(deficit, 1, int(cash // SEED_COST["TOMATO"]))
        if n > 0:
            buys.append(["BUY_SEED","TOMATO",n])
            cash -= n*SEED_COST["TOMATO"]

    if 21 <= day <= 27:
        deficit = max(0, crop_targets["CARROT"] - crops.get("CARROT",0) - seeds.get("CARROT",0))
        n = min(deficit, 3, int(cash // SEED_COST["CARROT"]))
        if n > 0:
            buys.append(["BUY_SEED","CARROT",n])
            cash -= n*SEED_COST["CARROT"]

    # Wheat is the residual/default crop, but avoid accumulating an unused pile.
    if day <= 26 and seeds.get("WHEAT",0) < 3:
        n = min(2, 3-seeds.get("WHEAT",0), int(cash // SEED_COST["WHEAT"]))
        if n > 0:
            buys.append(["BUY_SEED","WHEAT",n])
            cash -= n*SEED_COST["WHEAT"]

    # Preserve non-sell slots first; then use the rest for highest-value live sells.
    buys = buys[:10]
