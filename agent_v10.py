"""Kaggriculture v10-scaled

Scaled stable livestock baseline.

Changes from v9:
- scale herd from 3 cows + 2 sheep to 8 cows + 6 sheep;
- scale hired hands from 4 to 8;
- grow the herd in staged cohorts instead of all at once;
- buy up to two animals per order when cash and pasture slots permit;
- reserve cash dynamically from the live wheat price;
- prevent two workers from claiming different actions on the same tile.

No land expansion or strawberry yet.  This isolates whether production scale
and better task assignment close the gap before adding more systems.
"""
