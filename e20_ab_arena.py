...
CACHE = ROOT / "e20_ab_cache.json"
PREFIX = "@@E20GAME@@"

CANDIDATES = {
    "e11": "agent_e11_prvsiyan_frontier",
    "e20_early_sw": "agent_e20_prvsiyan_early_sw",
}
...
TELEMETRY_KEYS = (
    ...
    "activation_failures",
    "extra_hires",
    "seed_prefetch_requests",
    "seed_prefetch_activated",
    "native_seed_suppressed",
    "no_future_seed_order",
    "preplants",
    "waters",
...
def resolve(kind, name):
    ...
    return load_path(Path(ref), f"_e20_{name}")
...
def main():
    ...
    print("========== E11 vs E20 DIRECT ==========")
    rows, err = pair("candidate", "e20_early_sw", "candidate", "e11", seeds, cache)
    if err:
        raise SystemExit(err)
    w, d, l, margin = summary(rows)
    telemetry = telemetry_sum(rows)
    print(f"E20 vs E11: {w}-{d}-{l} meanMargin={margin:+.0f}")
    ...
    if not mechanism_live:
        print("\nSTOP: E20 mechanism did not fully activate (land + plant + water required).")
        print("Inspect telemetry before any larger evaluation.")
        Path("e20_ab_results.json").write_text(
    ...
    print("- Any reproducible direct or strong-holdout W-D-L regression rejects E20.")
    print("- Mean margin alone does not promote E20.")
