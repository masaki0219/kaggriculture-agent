...
def melon_maxxer(obs):
    ...
    # Sell rule unchanged from v4.
    melons_in_shed = shed.get("MELON", 0)
    if melons_in_shed > 0 and melon_price >= SELL_THRESHOLD:
        market.append(["SELL", "MELON", melons_in_shed])

    # v5 change:
    # buy the next seed while one seed is still available, so planting this turn
    # does not leave the next turn without seed inventory.
    melon_seeds = seeds.get("MELON", 0)
    if melon_seeds <= 1 and farm["money"] >= MELON_SEED_COST:
    ...
    elif tile is None and melon_seeds > 0:
        farmer = ["PLANT", "MELON"]
