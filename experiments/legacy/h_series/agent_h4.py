"""
agent_h4.py — repaired independent heuristic Kaggriculture agent
...
def _animal_goals(obs, farm, private, player):
    ...
    # Keep a cow floor, but do NOT force sheep into a collapsing wool market.
    if total >= 8:
        if goals["COW"] < 4:
        ...
        wool_ratio = prices["WOOL"]
        sheep_viable = (
            wool_ratio >= 0.55
            and score["SHEEP"] >= 0.55 * score["COW"]
        )
        if sheep_viable and goals["SHEEP"] < 2:
...
def _crop_targets(obs, farm, private, player):
    ...
    land_count = len(_get(farm, "unlocked_quadrants", []) or [])
    desired_herd = sum(_animal_goals(obs, farm, private, player).values())
    ...
    # Hard physical-capacity budget. Do not keep buying seed for acreage that
    # does not exist. Reserve one tile per desired animal structure plus a small
    # operational/weed buffer.
    physical_crop_capacity = max(
        8,
        land_count * 25 - desired_herd - 3,
    )

    # Feed backbone scales with herd, but cannot consume the entire crop budget.
    wheat_target = max(8, min(14, animal_count + 4))
    wheat_target = min(wheat_target, max(8, physical_crop_capacity - 2))

    premium_slots = max(0, physical_crop_capacity - wheat_target)
...
def _market_actions(obs, farm, private, player):
    ...
    # ---- expansion after realized sale proceeds ---------------------------
    # This is intentionally AFTER SELL orders. The market processes the listed
    # orders in sequence, so a profitable sale can finance land in the same turn.
    # h3 checked land only at hour 0, when cash was low, and therefore stayed on
    # one quadrant for all 30 days.
    herd_now = sum(owned.values())
    extra_land = max(0, len(_get(farm, "unlocked_quadrants", []) or []) - 1)
    ...
    if day <= 25 and (money >= 850 or day < 3):
