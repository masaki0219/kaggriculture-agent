"""
agent_h2.py — repaired independent heuristic Kaggriculture agent
...
def _hand_target(day):
    # Labour is rented fresh every day. Fibonacci hire cost explodes after
    # the 8th-10th hand, so scale only after cashflow exists.
    if day <= 4:
        return 8
    if day <= 11:
        return 9
...
def _crop_targets(obs, farm, private, player):
    ...
    # Feed backbone. 12-15 simultaneously active wheat plots are enough for a
    # large herd if maintained and watered, while still leaving room for premium.
    wheat_target = max(12, min(15, animal_count + 1))
    ...
    if day <= 17:
        targets["STRAWBERRY"] = 6
        targets["MELON"] = 4
        remaining = max(0, premium_slots - 10)
    else:
...
def _market_actions(obs, farm, private, player):
    ...
    if hour == 0 and extra_land < land_target and extra_land < len(LAND_PRICES):
        land_cost = LAND_PRICES[extra_land]
        threshold = land_cost + (700 if extra_land == 0 else 1200)
        if money >= threshold:
    ...
    if day <= 17:
        ...
        for need, animal in sorted(deficits, reverse=True):
            ...
            reserve = 900 + max(25, int(_get(prices, "WHEAT", 25) or 25)) * (animal_count + 4)
    ...
    if day <= 25:
        ...
        for _, _, crop, need in sorted(seed_needs, reverse=True):
            ...
            reserve = 500 if day < 18 else 250
            affordable = max(0, int((money - reserve) // unit))
            qty = min(12, need, affordable)
            if qty > 0:
