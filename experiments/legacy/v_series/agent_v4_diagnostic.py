from kaggle_environments.envs.kaggriculture.kaggriculture import CROPS

MELON_SEED_COST = CROPS["MELON"]["seed"]
MELON_MAX_YIELD_DAY = CROPS["MELON"]["max_yield_day"]

SELL_THRESHOLD = 200
MELON_BONUS_WATER_START_DAY = MELON_MAX_YIELD_DAY // 2

_STATS = {}


def reset_stats():
    global _STATS
    _STATS = {
        "MOVE": 0,
        "WATER": 0,
        "PLANT": 0,
        "HARVEST": 0,
        "PASS": 0,
        "TARGET_DISTANCE_SUM": 0,
        "TARGET_DISTANCE_COUNT": 0,
        "TARGET_BY_PURPOSE": {
            "harvest": {"count": 0, "distance_sum": 0},
            "water": {"count": 0, "distance_sum": 0},
            "plant": {"count": 0, "distance_sum": 0},
        },
    }


def get_stats():
    return _STATS


reset_stats()


def _step_toward(fx, fy, tx, ty):
    if fx > tx:
        return "WEST"
    if fx < tx:
        return "EAST"
    if fy > ty:
        return "NORTH"
    if fy < ty:
        return "SOUTH"
    return None


def _needs_water(tile, day):
    if tile.get("watered_today", False):
        return False

    planted_day = tile.get("planted_day")
    if planted_day is None:
        return False

    age = day - planted_day

    if age >= MELON_BONUS_WATER_START_DAY:
        return True

    consecutive_unwatered = tile.get("consecutive_unwatered", 0)
    return consecutive_unwatered >= 1


def _find_target_tile(farm, board_size, have_seed, day):
    fx, fy = farm["farmer"]
    candidates = []

    for y in range(board_size):
        for x in range(board_size):
            tile = farm["tiles"][y][x]

            if (
                isinstance(tile, dict)
                and tile.get("kind") == "PLANT"
                and tile.get("crop") == "MELON"
            ):
                purpose = None

                planted_day = tile.get("planted_day")
                if planted_day is not None:
                    age = day - planted_day

                    if (
                        age >= MELON_MAX_YIELD_DAY
                        and tile.get("yield_units", 0) > 0
                    ):
                        purpose = "harvest"

                if purpose is None and _needs_water(tile, day):
                    purpose = "water"

                if purpose:
                    candidates.append((x, y, purpose))

            elif tile is None and have_seed:
                candidates.append((x, y, "plant"))

    if not candidates:
        return None

    priority = {
        "harvest": 0,
        "water": 1,
        "plant": 2,
    }

    candidates.sort(
        key=lambda c: (
            priority[c[2]],
            abs(c[0] - fx) + abs(c[1] - fy),
        )
    )

    return candidates[0]


def _record_action(action_name):
    _STATS[action_name] += 1


def _record_target(fx, fy, target):
    tx, ty, purpose = target
    distance = abs(tx - fx) + abs(ty - fy)

    _STATS["TARGET_DISTANCE_SUM"] += distance
    _STATS["TARGET_DISTANCE_COUNT"] += 1

    bucket = _STATS["TARGET_BY_PURPOSE"][purpose]
    bucket["count"] += 1
    bucket["distance_sum"] += distance


def melon_maxxer(obs):
    farms = obs.get("farms", [])
    player = obs.get("player", 0)
    private = obs.get("private", {}) or {}

    if not farms or player >= len(farms):
        _record_action("PASS")
        return {
            "farmer": ["PASS"],
            "hands": [],
            "market": [],
        }

    farm = farms[player]
    board_size = len(farm["tiles"])
    fx, fy = farm["farmer"]
    tile = farm["tiles"][fy][fx]
    day = obs.get("day", 0)

    seeds = private.get("seeds", {})
    shed = private.get("shed", {})

    market_prices = (obs.get("market", {}) or {}).get("prices", {})
    melon_price = market_prices.get("MELON", 0)

    market = []

    melons_in_shed = shed.get("MELON", 0)
    if melons_in_shed > 0 and melon_price >= SELL_THRESHOLD:
        market.append(["SELL", "MELON", melons_in_shed])

    if (
        seeds.get("MELON", 0) == 0
        and farm["money"] >= MELON_SEED_COST
    ):
        market.append(["BUY_SEED", "MELON", 1])

    farmer = ["PASS"]
    chosen_target = None

    if (
        isinstance(tile, dict)
        and tile.get("kind") == "PLANT"
        and tile.get("crop") == "MELON"
    ):
        planted_day = tile.get("planted_day")
        age = day - planted_day if planted_day is not None else -1

        if (
            age >= MELON_MAX_YIELD_DAY
            and tile.get("yield_units", 0) > 0
        ):
            farmer = ["HARVEST"]

        elif _needs_water(tile, day):
            farmer = ["WATER"]

        else:
            target = _find_target_tile(
                farm,
                board_size,
                seeds.get("MELON", 0) > 0,
                day,
            )

            if target:
                chosen_target = target
                step = _step_toward(
                    fx,
                    fy,
                    target[0],
                    target[1],
                )
                if step:
                    farmer = [step]

    elif tile is None and seeds.get("MELON", 0) > 0:
        farmer = ["PLANT", "MELON"]

    else:
        target = _find_target_tile(
            farm,
            board_size,
            seeds.get("MELON", 0) > 0,
            day,
        )

        if target:
            chosen_target = target
            step = _step_toward(
                fx,
                fy,
                target[0],
                target[1],
            )
            if step:
                farmer = [step]

    action = farmer[0]

    if action in ("NORTH", "SOUTH", "EAST", "WEST"):
        _record_action("MOVE")
        if chosen_target:
            _record_target(fx, fy, chosen_target)
    elif action == "WATER":
        _record_action("WATER")
    elif action == "PLANT":
        _record_action("PLANT")
    elif action == "HARVEST":
        _record_action("HARVEST")
    else:
        _record_action("PASS")

    return {
        "farmer": farmer,
        "hands": [],
        "market": market,
    }
