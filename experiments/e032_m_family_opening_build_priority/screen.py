...
def child(args):
    ...
    agents=[e32,opp] if args.e32_seat==0 else [opp,e32]
    env=make("kaggriculture",configuration={"episodeSteps":720,"seed":args.seed},debug=False)
    env.run(agents)
    a=float(env.steps[-1][args.e32_seat].get("reward"))
    b=float(env.steps[-1][1-args.e32_seat].get("reward"))
    print(PREFIX+json.dumps({
        "opponent":args.opponent,
        "seed":args.seed,
        "e32_seat":args.e32_seat,
        "e32_reward":a,
        "opponent_reward":b,
        "outcome":"W" if a>b else "L" if a<b else "D",
        "trajectory":{str(s):diag_at(env,args.e32_seat,s) for s in CHECKPOINTS},
    },ensure_ascii=False))

def run_one(opponent,seed,seat):
    p=subprocess.run(
        [sys.executable,str(Path(__file__).resolve()),"--child",
         "--opponent",opponent,"--seed",str(seed),"--e32-seat",str(seat)],
    ...
    return {
        "opponent":opponent,"seed":seed,"e32_seat":seat,
...
def main():
    ...
    ap.add_argument("--e32-seat",type=int,choices=(0,1))
    args=ap.parse_args()
    if args.child:
        child(args)
    else:
